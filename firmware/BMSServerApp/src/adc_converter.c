...
uint16_t adc_convert_cell_voltage(const uint16_t adc_value) {
	float voltage = (float)adc_value * REF_VOLT_ADC/ ADC_MAX_VALUE;
	return (uint16_t)(voltage * 1000);
}

/*
	LM35 Specifications:
	Output voltage = Temperature in deg per 10 mV
	At at minus 55 deg: VOUT = minus 550 mV
...
int16_t adc_convert_battery_temp(const uint16_t output_adc, const uint16_t diode_adc){
    ...
    // Convert the voltage to temperature in Celsius (LM35 outputs 10 mV/Ã‚Â°C or 0.01V/Ã‚Â°C)
...
/*
	...
	The voltage drop across the shunt would be: shunt=Imax*shunt=10AÃ—0.01=0.1V
	...
	m is the slope (calibration gain).
	b is the offset (calibration offset).
*/
uint16_t adc_convert_battery_current(const uint16_t adc_value){
    ...
    float current = (CURRENT_CALIBRATION_GAIN * (shunt_voltage / R_SHUNT)) + CURRENT_CALIBRATION_OFFSET; // Calculate current with calibration
    return current;
}
